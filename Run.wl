...
If[debug,
	SetDirectory["/home/andrzej/Documents/Uczelnia/Anizotropie/FunctionalRenormalization"];
...
If[debug,
	...
	task = "ON"; (* ON/ Z4 / tricritical/ regulator /lowd / ising / ising2 *)
	configuration = 10;
	ON = True;
...
gridSizeDict = <|"ON" -> 120, "Z4" -> 120, "regulator" -> 120, "lowd" -> 120, "ising" -> 300, "ising2" -> 300, "tricritical" -> 100|>;
...
potentialMinimumDict = <|"ON" -> Floor[50/100*gridSize], "Z4" -> Floor[130/200*gridSize], 
...
IntegralConfigurations = {{0,1/16,40},{1/16,9/10,60},{9/10,3/2,40}};
Protect[IntegralConfigurations];

eqlist = IntegrateEquations[integrands];

alphaStep = 1/2;
initialAlphas = 2+Sign[constants["alpha"]-2]*Range[0,Abs[constants["alpha"]-2]/alphaStep]*alphaStep;
...
(* ::Subsection::Closed:: *)
(*Low-T task computation*)


a = Import[guessfile];


b = Import["~/Desktop/test.csv"];


vddd = 2/3 (2.^(-1-d) Pi^(-d/2))/Gamma[d/2] /. d->b[[1,2]];


vpoints = b[[2;;-1,1]]; zspoints = b[[2;;-1,2]]; zppoints = b[[2;;-1,3]];
\[Rho]max = Rationalize[b[[1,10]]/vddd,1/10000];
guess = Append[Flatten[Table[{v[i-1]->vpoints[[i]],zs[i-1]->zspoints[[i]],zp[i-1]->zppoints[[i]]},{i,1,Length[vpoints]}]], \[Eta]->0.4171];
guess = DeleteCases[guess, zs[0]->f_];
guess = DeleteCases[guess, zp[0]->f_];


Unprotect[gridSize];
gridSize = Length[vpoints]-1;
Protect[gridSize];


integrands = integrandsListIsotropic/.n->13/10;


eqs = IntegrateEquations[integrands];


fp = FindFixedPoint[eqs, guess, ConstRep[17/10,\[Rho]max,2]];
...
eqs[[1;;3]]/.guess/.ConstRep[17/10,\[Rho]max,2]


PlotFixedPoint[fp,\[Rho]max]


sm = StabilityMatrix[integrands,fp, ConstRep[17/10,\[Rho]max,2]];
...
schemes = {"smooth", "smooth_strict","exponential", "exponential_strict","litim", "litim_strict", "smooth_LPA"};
allExponents = <| |>;
...
GetLegend[scheme_]:=Block[{label,regulator,underscore,labelDict, mode=" ansatz", approx=""},
...
If[Length[underscore] == 0, 
	...
	If[StringTake[scheme,-3;;-1] == "LPA",
		approx = " LPA"];
	If[Length[underscore] == 2 || StringTake[scheme,-3;;-1] != "LPA", 
		mode = " strict";
	];
];
	
label = labelDict[regulator]<>mode<>approx;
Return[label]];
...
dims = Keys[allExponents["smooth_LPA",y1]];
...
ListPlot[Re[nested[25/10,e2]]]


Re[nested[26/10,e2]]/nested[26/10,e1]


(Re[nested[26/10,e2]]/nested[26/10,e1])[21/10]
...
ListPlot[(Re[nested[25/10,e2]]/nested[25/10,e1])]


ListPlot[(Re[nested[26/10,e2]]/nested[26/10,e1])]


ListPlot[Re[nested[21/10,e2]]/nested[21/10,e1]]
...
dd=3;
aa=2;
fp = KeySelect[fps,MatchQ[{dd,aa,rr_}]][[1]];
rr = Keys[KeySelect[fps,MatchQ[{dd,aa,rr_}]]][[1,3]];
...
guess = AdjustNumberOfPoints[fp, gridSize, zNormalization, rr];


guess = Append[guess[[1;;2 gridSize+1]], guess[[-1]]];


ListPlot[dev]


PlotFixedPoint[guess, rr]


newfp = FindFixedPoint[eqs,guess,ConstRep[dd,aa,rr]];


sm = StabilityMatrix[integrands, fp, ConstRep[dd,aa,rr]];
...
{fps,exps} = Import["ON_functional_smooth backup/fixed_points.wdx"];
...
threshold[d_,i_,m_,z_,eta_:0] := (((2-eta)r[#^2]-2#^2 r[#^2]) #^(d-1) /(m+z #^2+r[#^2])^i /.regulatorReplacement/.\[Alpha]->2)&
threshold2[d_,i_,m_,z_] := ((r[#]-# r[#] #^(d/2-1)/2) /(m+z #+r[#])^i /.regulatorReplacement/.\[Alpha]->2)&
...
IntegralConfigurations = {{0,1/16,30},{1/16,9/10,40},{9/10,3/2,30}};
		...
		configurationsq2 = { { 0, b, 20 }, { .5, 5., 40 }, { 5., 49., 20 } };
		configurationsq = { { 0, 0.1, 40 }, { 0.1, 3., 40 }, { 3., 7., 20 } };


config = configurationsq2[[2]];
...
TotalDiff[f_, configs_] := Block[{ints, total},
ints = Map[DiffExpand[f,#]&, configs];
total = (Total[ints[[All,1]]]+NIntegrate[f[q],{q,configs[[-1,3]],100},WorkingPrecision->40, AccuracyGoal->16, PrecisionGoal->16, MaxRecursion->50]);
Return[{total,Abs[1 - total/Total[ints[[All,2]]] ]}];]


TotalDiff[, { { 0, .02, 30 }, { 0.02, .5, 30 }, { .5, 1, 30 }, { 1, 2, 30 }, { 2, 4, 20 } }]


TotalDiff[threshold[19/10, 5, -199/100, 1,2/10], { { 0, .02, 30 }, { 0.02, .5, 30 }, { .5, 1, 30 }, { 1, 2, 30 }, { 2, 4, 20 } }]


configs = Tuples[{{17/10,19/10}, Range[2,5], {0, -19/10, -195/100}, {1,1/10,1/30}, {4/10}}];


configs = Select[configs, #[[3]]>=0 || #[[4]] >=1&];


functions = MapApply[threshold,configs];


Plot[functions[[1]][y],{y,0,6}]


res = Map[TotalDiff[#, { { 0, .005, 30 }, { 0.005, .5, 30 }, { .5, 1, 30 }, { 1, 2, 30 }, { 2, 4, 20 } }][[2]]&, functions];
...
r[y]/.regulatorReplacement


FindMinimum[(y \[Alpha])/(-1+E^y) + z y /. {\[Alpha]->2, z->8/10}, y]


diffs = Association[Map[#->TotalDiff[threshold2[2, 5, -193/100, 8/10], { { 0, #, 20 }, { #, 2, 40 }, { 2, 15, 25 } }]&, 10^(-Range[30]/10)]];
ListLogPlot[diffs]


ints = Association[Map[#->NIntegrate[threshold2[2, 5, -2*999/1000, 1][q],{q,config[[1]],#},WorkingPrecision->40, AccuracyGoal->16, PrecisionGoal->16, MaxRecursion->50]-
	GLIntegral[threshold2[2, 5, -2*999/1000, 1],config[[1]],#,config[[3]]] &, Range[100]/1000]];


ListLogPlot[Abs[ints]]


sparse = 20;
integrands1 = Flatten[Table[NumericDerivatives[myintegrands/.{n->15/10, \[Rho] -> (1+ sparse (i-1)) eps}/.regulatorReplacement]//.ConstRep[dd,rr,aa]/.test,{i,1,gridSize/sparse}]];
integrands[q_] = integrands1 /.y->q;
...
IntegralConfigurations = {{0,1/4,25},{1/4,125/100,40},{125/100,3,20}};
...
PMS[data_,func_:Re] := Block[{norm=Normal[data], pairs, inter,pms, amin, amax},
amin = Min[norm[[1]]];
amax = Max[norm[[1]]];
pairs =  Map[{norm[[#,1]],func[norm[[#,2]]]}&,Range[Length[norm]]];

inter = Interpolation[pairs];
pms = FindRoot[inter'[\[Alpha]]==0,{\[Alpha],2, amin, amax}];

If[((\[Alpha]/.pms) <= amin) || ((\[Alpha]/.pms) >= amax), Return[Null]]; 
...
GetPotentialMinimum[action_] := Block[{vdata,pairs,inter, gridSize, min},
vdata = Normal[Select[action,MatchQ[v[i_]->f_]]];
pairs = Transpose[{vdata[[All,1,1]],vdata[[All,2]]}];
inter = Interpolation[pairs];
gridSize = Length[vdata]-1;
min = x/gridSize /. FindRoot[inter[x],{x,gridSize/2}];
Return[min];
]


normalisedMinima = Association[Map[#[[1;;3]]->#[[4]] * GetPotentialMinimum[fps[#]]&,Keys[fps]]];


CycleKeys[ass_] := KeyMap[Permute[#,{3,1,2}]&,ass];


partnestedMinima = OneLevelNest[CycleKeys[normalisedMinima]];


pmsminima = Map[#[2]&,partnestedMinima];


pmsminima = Association[DeleteCases[pmsminima,Missing["KeyAbsent",2]]];


pmsminima = Map[Min[Values[#]]&,partnestedMinima];


gdata = NestAssociation[1/pmsminima];
p = ListPlot[Values[gdata], PlotLegends->Keys[gdata]];


Solve[0==(\[Epsilon] t + (2-n) t^2 ( 1+t + (n+2) t^2/4 + t^3(-n^2/12 -n/2+4+(n-4)(7/3+3 Zeta[3]/2) + (5+3 Zeta[3])/2)))/.{\[Epsilon]->-.2,n->2.1},t]


tsol = t/. Solve[0==\[Epsilon] t + (2-n) t^2 ( 1+t + (n+2) t^2/4 + t^3(-n^2/12 -n/2+4+(n-4)(7/3+3 Zeta[3]/2) + (5+3 Zeta[3])/2)),t];


tsolm = t/. Solve[0==\[Epsilon] t + (2-n) t^2 ( 1+t + (n+2) t^2/4 + t^3(-n^2/12 -n/2+4+(n-4)(7/3+3 Zeta[3]/2) + (5+3 Zeta[3])/2))/.n->1.5,t,Reals][[2;;3]];


Plot3D[tsol,{\[Epsilon],-.5,.5},{n,2.001,2.5},PlotRange->{0,10}]


Plot3D[tsol,{\[Epsilon],-.5,.5},{n,1.5,1.999}]


tsol0 = t/. Solve[0==\[Epsilon] t + (2-n) t^2 ,t][[2]];


gsel = Normal[gdata[5/2][[-3;;-1]]];
pairs = Transpose[{gsel[[All,1]], gsel[[All,2]]}];
NonlinearModelFit[pairs, A (x-2),A, x]


0.020872896942402186`/(1/2)


Series[tsol /.n->1.5,{\[Epsilon],0,1}]


Plot[tsolm/.{n->{3/5},\[Epsilon]->d-2},{d,1.6,2.2}]


Show[p, Plot[{tsol[[3]] 0.6860489179900288`/2 /.{n->2.5,\[Epsilon]->d-2},tsol[[4]] 0.6860489179900288`/2 /.{n->1.5,\[Epsilon]->d-2},tsol[[4]] 0.6860489179900288`/2 /.{n->1.7,\[Epsilon]->d-2}},{d,1.775,2.2}]]


flattened = UnnestAssociation[nested];
aDependence = OneLevelNest[KeyMap[Permute[#,{3,1,2}]&,flattened]];
...
sel = KeySelect[fps,MatchQ[{25/10,11/10,16/10, r_}]];
{aa, nn, dd, rr} = Keys[sel][[1]];
myfp = sel[[1]];
myfp[[1;;-2,2]] /= aa;


selexps = KeySelect[exps, MatchQ[{e1,a_,b_,c_}]];


nested = NestAssociation[selexps][e1];


partnest = OneLevelNest[selexps];


SelectMonotonous[ass_]:=Block[{newass=<||>,keys=Keys[ass],i},
For[i=1, i<=Length[ass],i++,
If[Im[ass[[i]]]==0 && ass[[i]]>0 && (i==1||ass[[i-1]]>ass[[i]]), 
	newass[keys[[i]]]=ass[[i]],
	Break[]];
];
Return[newass];
...
selected = Map[SelectMonotonous, partnest];
nested = NestAssociation[selected][e1];
...
PMS[data_,func_:Re] := Block[{norm=Normal[data], pairs, inter,pms, amin, amax,GetMin, maxXdist=.5, i},
amin = Min[norm[[All,1]]];
amax = Max[norm[[All,1]]];
pairs =  Map[{norm[[#,1]],func[norm[[#,2]]]}&,Range[Length[norm]]];

For[i=1,i<Length[norm],i++,
	If[Abs[norm[[i,1]]-norm[[i+1,1]]] > maxXdist, Return[Null]];
];

If[Length[norm] <= 4, Return[Null]];

inter = Interpolation[pairs, InterpolationOrder->4];

GetMin[interpol_, weightMin_:1/2, weightMax_:1/2]:=Block[{min},
min = Quiet[FindRoot[interpol'[\[Alpha]]==0,{\[Alpha],(amin *weightMin +amax*weightMax)/(weightMax+weightMin), amin, amax}]];
If[((\[Alpha]/.min) <= amin) || ((\[Alpha]/.min) >= amax), Return[Null]];
Return[\[Alpha]/.min]; 
];

pms = GetMin[inter,1,1];
If[NumberQ[pms], Return[inter[pms]]]; 
pms = GetMin[inter,1,2];
If[NumberQ[pms], Return[inter[pms]]]; 
pms = GetMin[inter,2,1];
If[NumberQ[pms], Return[inter[pms]]]; 
Return[ Null];
]


pmsdata = Map[PMS,aDependence];


pmsdata2 = Association[DeleteCases[Normal[pmsdata], k_->Null]];
pmsdata2 = Association[DeleteCases[Normal[pmsdata2], k_->a_/;a<0]];
pmsdata2 = NestAssociation[pmsdata2];


cardyeigenvals = {1/(6144 \[Pi]^3) (-4076 \[Pi]^3+1722 n \[Pi]^3+168 n^2 \[Pi]^3-5 n^3 \[Pi]^3-3072 \[Pi]^3 \[Epsilon]+540 \[Pi]^3 Zeta[3]-450 n \[Pi]^3 Zeta[3]+90 n^2 \[Pi]^3 Zeta[3]-\[Sqrt]((4076 \[Pi]^3-1722 n \[Pi]^3-168 n^2 \[Pi]^3+5 n^3 \[Pi]^3+3072 \[Pi]^3 \[Epsilon]-540 \[Pi]^3 Zeta[3]+450 n \[Pi]^3 Zeta[3]-90 n^2 \[Pi]^3 Zeta[3])^2-4 (-23347200 \[Pi]^6+10838016 n \[Pi]^6+442368 n^2 \[Pi]^6-12288 n^3 \[Pi]^6-37748736 \[Pi]^6 \[Epsilon]+1327104 \[Pi]^6 Zeta[3]-1105920 n \[Pi]^6 Zeta[3]+221184 n^2 \[Pi]^6 Zeta[3]))),1/(6144 \[Pi]^3) (-4076 \[Pi]^3+1722 n \[Pi]^3+168 n^2 \[Pi]^3-5 n^3 \[Pi]^3-3072 \[Pi]^3 \[Epsilon]+540 \[Pi]^3 Zeta[3]-450 n \[Pi]^3 Zeta[3]+90 n^2 \[Pi]^3 Zeta[3]+\[Sqrt]((4076 \[Pi]^3-1722 n \[Pi]^3-168 n^2 \[Pi]^3+5 n^3 \[Pi]^3+3072 \[Pi]^3 \[Epsilon]-540 \[Pi]^3 Zeta[3]+450 n \[Pi]^3 Zeta[3]-90 n^2 \[Pi]^3 Zeta[3])^2-4 (-23347200 \[Pi]^6+10838016 n \[Pi]^6+442368 n^2 \[Pi]^6-12288 n^3 \[Pi]^6-37748736 \[Pi]^6 \[Epsilon]+1327104 \[Pi]^6 Zeta[3]-1105920 n \[Pi]^6 Zeta[3]+221184 n^2 \[Pi]^6 Zeta[3])))};


ns = Reverse[{25/10, 2,19/10,18/10}];


cardyfunc = cardyeigenvals[[2]] /. {n->ns,\[Epsilon]->d-2};


pmsselected = KeySelect[pmsdata2,MemberQ[ns,#]&];
MarkerList=Reverse[{"\[FilledCircle]","\[FilledSquare]","\[FilledUpTriangle]","\[FilledDownTriangle]"}];
markers[s_] := Tuples[{MarkerList,{s}}];
pointdata = ListPlot[Reverse[Values[pmsselected]], PlotMarkers->markers[20]];


data = Map[Transpose[{Keys[#][[-8;;-1]],Values[#][[-8;;-1]]}]&,pmsselected];


ListPlot[data[[3]]]


NonlinearModelFit[data[[4]], {A (x-x0)^b, 0<b<1.1}, {{A,1},{x0,1},{b,1/2}}, x]


Show[Plot[Normal[%259],{x,1.8,2.2}],ListPlot[data[[3]]]]


Map[NonlinearModelFit[#, A (x-x0)^b, {{A,1},{x0,1},{b,1/2}}, x]&, data]


cardy2= ListLinePlot[{{{I,I}},{{I,I}},{{I,I}},{{2.`,6.967946809716219`*^-16},{2.01`,0.01020077041116713`},{2.02`,0.020804504539552744`},{2.03`,0.031810152666754604`},{2.04`,0.04321327489346961`},{2.05`,0.055006937992019125`},{2.06`,0.06718242942286884`},{2.07`,0.07972981751046858`},{2.08`,0.09263838533923692`},{2.09`,0.10589696284999177`},{2.1`,0.11949417798115923`},{2.11`,0.1334186441094455`},{2.12`,0.1476590977942626`},{2.13`,0.16220449803243997`},{2.14`,0.1770440958981195`},{2.15`,0.19216748154156887`},{2.16`,0.20756461399400195`},{2.17`,0.2232258380126767`},{2.18`,0.2391418912450805`},{2.19`,0.25530390424165117`},{2.2`,0.27170339526262843`}}}, 
	PlotTheme->"Monochrome",PlotStyle->Directive[StandardPlotColours[[4]],AbsoluteThickness[4]],PlotMarkers->None];


jointplot=Show[ 
	Plot[cardyfunc,{d,1,3}, MaxRecursion->10, 
	PlotTheme->"Monochrome", PlotStyle->MapThread[Directive,{StandardPlotColours,Map[AbsoluteThickness[4]&,Range[Length[StandardPlotColours]]]}],
		PlotLegends->Placed[LineLegend[Map["N="<>ToString[NumberForm[N[#],{2,1}]]&,ns], LegendLayout->{"Row",1}, LegendMarkers->markers[28], 
		LabelStyle->Directive[Black, 24]], Bottom],
		LabelStyle->Directive[Black, 24],Frame->True, FrameLabel->{"d=2+\[Epsilon]", Rotate[Subscript["e",1],-Pi/2]}],
	cardy2,
	pointdata,
	PlotRange->{{1.9,2.4},{0,1.3}},ImageSize->Large,
	GridLines->Automatic, GridLinesStyle->Gray];


Export["/home/andrzej/Documents/Uczelnia/Papers/Lau line (2022)/e1_my.png",jointplot];


Map[Min[Keys[#]]&,pmsdata2]


TransposeAssociation[pmsdata2]


Keys[Normal[pmsdata2][[1]][[2]]]


laudata = Association[Map[Min[Keys[#[[2]]]]->#[[1]]&,Normal[pmsdata2]]];
...
laudata2 = {{1.4,1.1},{1.45,1.2},{1.55,1.3},{1.7,1.4},{1.75,1.5},{1.8,1.6},{1.85,1.7},{1.9,1.8},{1.95,1.9},{2,2},{2,2.5},{2,3}};
...
PlotAssociation[data[\[Eta],20/10]]
...
sel = KeySelect[fps, #[[1]]==15/10 &];


nest = NestAssociation[sel][15/10];


PlotFixedPoint[nest[14/10][[-3,1]],27711063136784789642746066468524931348905802131/1600000000000000000000000000000000000000000000]
...
pair = KeySelect[fps,MatchQ[{3,2,r_}]];
fp = pair[[1]];
rr = Keys[pair][[1,3]];


vvals = fp[[1;;101,2]];
vxs = rr*Range[0,100]/100;
zsvals = fp[[102;;201,2]];
zsxs = rr*Range[1,100]/100;
zpvals = fp[[202;;301,2]];
zpxs = rr*Range[1,100]/100;


vdata = Transpose[{vxs,vvals}];
zsdata = Transpose[{zsxs,zsvals}];
zpdata = Transpose[{zpxs,zpvals}];


vfunc = Interpolation[vdata, InterpolationOrder->4]; zsfunc = Interpolation[zsdata, InterpolationOrder->4]; zpfunc = Interpolation[zpdata, InterpolationOrder->4];
\[Kappa] = \[Rho]/.FindRoot[vfunc[\[Rho]]==0,{\[Rho],0.8 rr}][[1]];


nv = 2;
nzs = 2;
nzp = 2;
mv = NonlinearModelFit[vdata, Sum[a[n] (x-\[Kappa])^n/n!,{n,0,nv}],Table[a[n],{n,0,nv}],x];
mzs = NonlinearModelFit[zsdata, Sum[a[n] (x-\[Kappa])^n/n!,{n,0,nzs}],Table[a[n],{n,0,nzs}],x];
mzp = NonlinearModelFit[zpdata, Sum[a[n] (x-\[Kappa])^n/n!,{n,0,nzp}],Table[a[n],{n,0,nzp}],x];


Show[ListPlot[vdata],Plot[Normal[mv],{x,0,rr}]]


Show[ListPlot[zsdata],Plot[Normal[mzs],{x,0,rr}]]


Show[ListPlot[zpdata],Plot[Normal[mzp],{x,0,rr}]]
